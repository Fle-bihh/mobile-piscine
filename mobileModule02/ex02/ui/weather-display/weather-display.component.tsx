...
const CurrentWeather = ({ weather }: { weather: WeatherData | null }) => {
	...
	const closestIndex = weather.hourly.time.findIndex((time) => time.startsWith(currentTime));

	const index = closestIndex !== -1 ? closestIndex : weather.hourly.time.length - 1;

	const temperature = weather.hourly.temperature_2m[index];
	const windSpeed = weather.hourly.windspeed_10m[index];
	const weatherDescription = getWeatherDescription(weather.hourly.weathercode[index]);
