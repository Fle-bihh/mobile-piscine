...
export const getTabBarLabelStyles = ({ isActive, icon }: TabBarLabelStylesProps) => {
	const iconName = getAppIconName(icon);
	const iconSize = 24;
	const iconColor = isActive ? EPalette.Primary : EPalette.TextSecondary;
	const styles = StyleSheet.create({
		...
		label: {
			color: isActive ? EPalette.Primary : EPalette.TextSecondary,
			fontSize: 10,
